...
exports.searchUserByUserid = "select `userId` from `user` where `userId` = ? and `userId` not in (select userId from blocks where targetId = ? and userId = ?);"
exports.searchUserByNickname = "select `userId` from `user` where `userId` = ? and `userId` not in (select userId from blocks join `user` on `blocks`.`targetId` = `user`.`userId` where `nickName` = ? and userId = ?);"
