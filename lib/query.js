...
exports.searchUserByUserid = "select `userId` from `user` where `userId` = ? and not exists (select `userId` from `blocks` where `targetId` = ? and `userId` = ?);"
exports.searchUserByNickname = "select `nickName` from `user` where `nickName` = ? and not exists (select `blocks`.`userId` from `blocks` join `user` on `blocks`.`targetId` = `user`.`userId` where `nickName` = ? and `user`.`userId` = ?);"
...
exports.listGroupParticipant = "select `group`.`gid`, `name`, `explanation`, `color`, `userId` as 'creator' from `group` left outer join `group_participation` on `group`.`gid` = group_participation.gid where `group`.`gid` = ? and `isAccepted` = true;"
...
exports.getLocation = "select `nickName`, `latitude`, `longitude` from `participant_coordination` where `pid` = ?;"
