...
// plan
exports.newPlan = "insert into `plan` (`name`, `startTime`, endTime, location, category) values(?, str_to_date(?, '%Y-%m-%d %H:%i:%s'), str_to_date(?, '%Y-%m-%d %H:%i:%s'), ?, ?);" + "insert into `plan_participation` (`pid`, `userId`, `plan_privilege`) values(last_insert_id(), ?, 1);" + "insert into `group_plan_belongs` (`pid`, `gid`) values(last_insert_id(), ?);"
...
exports.getAvatarPath = "update `user` set `image` = ? where `userId` = ?"
exports.setAvatarPath = "select `image` from `user` where `userId` = ?"
