...
exports.searchUserByUserid = "select `userId`, `nickName`, if(`isOnline`, 'true', 'false') as 'isOnline', if(`isAttend`, 'true', 'false') as 'isAttend' from `user` where `userId` = ? and not exists (select `userId` from `blocks` where `targetId` = ? and `userId` = ?);"
exports.searchUserByNickname = "select `userId`, `nickName`, if(`isOnline`, 'true', 'false') as 'isOnline', if(`isAttend`, 'true', 'false') as 'isAttend' from `user` where `nickName` = ? and not exists (select `blocks`.`userId` from `blocks` join `user` on `blocks`.`targetId` = `user`.`userId` where `nickName` = ? and `user`.`userId` = ?);"
...
exports.inviteGroup = "insert into `group_participation` (`userId`, `gid`) select ?, ? from dual where not exists (select * from `group_participation` where `userId` = ? and `gid` = ?);"
...
exports.detailsGroup = "select `gid`, `name`, `explanation`, `color`, `userId` as 'creator' from `groupinfo` where `gid` = ? and `group_privilege` = 1;"
exports.getGroupName = "select `name` from `group` where `gid`= ?"
exports.listGroupParticipant = "select `group_participation`.`userId`, `nickName`, if(`isOnline`, 'true', 'false') as 'isOnline', if(`isAttend`, 'true', 'false') as 'isAttend' from `user` left outer join `group_participation` on `group_participation`.`userId` = `user`.`userId` where `group_participation`.`gid` = ? and `isAccepted` = true;"
exports.countGroupParticipant = "select count(*) as 'personnel' from `group_participation` where `gid` = ? and `isAccepted` = true"

// map
exports.setMyLocation = "update `plan_participation` set `latitude` = ?, `longitude` = ? where `userId` = ?;"
exports.getLocation = "select distinct `nickName`, `latitude`, `longitude` from `participant_coordination` where `pid` = ? and `isAccepted` = true and `latitude` is not null and `longitude` is not null;"
...
exports.listMemo = "select `memo`, `nickName` as 'writer' from `plan_participation` join `user` on `plan_participation`.`userId` = `user`.`userId` where `pid` = ? and `memo` is not null;"
...
exports.invitePlan = "insert into plan_participation(pid, userId, isAccepted) select ?, ?, ? from dual where not exists (select * from `user` where `userId` = ? and `planInviteOption` = true);"
...
exports.getPlace = "select `location` from `plan` where `pid` = ?"
exports.listFriendPlan = "select plan.pid, name, date_format(`startTime`, '%Y-%m-%d %H:%i:%s') as 'startTime', date_format(endTime, '%Y-%m-%d %H:%i:%s') as endTime, location, category, if(isPublic, 'true', 'false') as isPublic from plan left outer join plan_participation on plan_participation.pid = plan.pid where plan_participation.userId= ? and plan.isDone=false and plan_participation.isCurrent=true and isAccepted = true and plan.isPublic = true;"
exports.checkPlanInviteOption = "select `planInviteOption` from `user` where `userId` = ?"
exports.checkFriendInviteOption = "select `friendInviteOption` from `user` where `userId` = ?"
...
exports.getProfile = "select userId, nickName, name, age, if(gender, 'true', 'false') as gender from user where userId=?"
...
exports.setToken = "update ignore `user` set token = ? where userId = ?;"
