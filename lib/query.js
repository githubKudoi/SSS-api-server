...
exports.invitePlan = "insert into plan_participation(pid,userid) values(?,?);"             // 약속 id, 대상 사용자 리스트 -> 약속 초대(약속과 link)
...
exports.setPublicPlan = "update plan_participation set is_public=1 where userid=? and pid=?;"          // 사용자 id, 약속 id -> 약속 공개처리
exports.setPrivatePlan = "update plan_participation set is_public=0 where userid=? and pid=?;"         // 사용자 id, 약속 id -> 약속 비공개처리
...
exports.listPrevPlan = "select * from plan left outer join plan_participation on plan_participation.pid = plan.pid where plan_participation.userid=? and plan.is_done=true;"           // 사용자 id -> 이전 약속 리스트
...
exports.newProfile = "insert into user(userid,nickname,name,age,gender,profile_image) values(?,?,?,?,?,?)"             // 사용자 id, 별명, 실명, 나이, 성별, 사진 -> 신규 생성
